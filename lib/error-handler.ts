...
export class ErrorHandler {
  private static handlers: Map<ErrorType, (error: StructuredError) => Promise<void>> = new Map([
  ...
  static async handle(
    ...
    // 기본 핸들러 실행
    if (!options.silent) {
      const handler = this.handlers.get(structuredError.type) || this.handleUnknownError
  ...
  private static async handleAuthenticationError(error: StructuredError): Promise<void> {
    // 자동 토큰 갱신 시도 및 리디렉션
    await handleUnauthorizedError()
  }

  private static async handleAuthorizationError(error: StructuredError): Promise<void> {
    // 권한 부족 처리 (리디렉션 없음)
    console.warn('Authorization failed:', error)
  }

  private static async handleValidationError(error: StructuredError): Promise<void> {
    // 유효성 검사 에러 처리
    console.warn('Validation error:', error)
  }

  private static async handleNotFoundError(error: StructuredError): Promise<void> {
    // 404 에러 처리
    console.warn('Resource not found:', error)
  }

  private static async handleConflictError(error: StructuredError): Promise<void> {
    // 충돌 에러 처리 (중복 데이터 등)
    console.warn('Resource conflict:', error)
  }

  private static async handleServerError(error: StructuredError): Promise<void> {
    // 서버 에러 처리
    console.error('Server error:', error)
  }

  private static async handleNetworkError(error: StructuredError): Promise<void> {
    // 네트워크 에러 처리
    console.error('Network error:', error)
  }

  private static async handleUnknownError(error: StructuredError): Promise<void> {
    // 알 수 없는 에러 처리
    console.error('Unknown error:', error)
  }

  private static showErrorToast(error: StructuredError): void {
  ...
  private static logError(error: StructuredError): void {
    ...
    if (error.type === ErrorType.SERVER_ERROR || error.type === ErrorType.UNKNOWN_ERROR) {
 ...
 */
export function handleApiError(error: unknown, options?: ErrorHandlingOptions): never {
