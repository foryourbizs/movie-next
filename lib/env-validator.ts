...
export class EnvironmentValidator {
  ...
  constructor(schema: EnvironmentSchema, processEnv: Record<string, string | undefined> = process.env) {
    this.schema = schema
 ...
 * 환경변수 검증 실행 및 결과 반환
 */
export function validateEnvironment(schema?: EnvironmentSchema): ValidationResult {
 ...
 */
export function validateEnvironmentOrExit(schema?: EnvironmentSchema): Record<string, string | number | boolean> {
  // 클라이언트에서는 실행하지 않음
  if (typeof window !== 'undefined') {
    console.warn('validateEnvironmentOrExit should only be called on server side')
    return {}
  }
 ...
 * 타입 안전한 환경변수 접근
 */
export function createEnvAccessor<T extends Record<string, string | number | boolean>>(
  validatedValues: T
) {
  return {
    ...
    getNumber(key: keyof T): number {
      ...
      if (isNaN(parsed)) {
        throw new Error(`Environment variable '${String(key)}' is not a valid number: ${value}`)
      }
      return parsed
    },
    ...
    getUrl(key: keyof T): URL {
      ...
      try {
        ...
        throw new Error(`Environment variable '${String(key)}' is not a valid URL: ${value}`)
      }
    },
  }
}
